...
class FilterComponent extends HTMLElement {
	...
	injectSearchResults(url) {
		fetch(url)
			...
			.then((data) => {
				...
				document.querySelector(".search__page-tab[for='products']").innerHTML = "Products " + searchResults.innerHTML;
			});
	}
	...
	buildUrl() {
		...
		let url = `${windowLocation.origin}/${searchTerm}${sortBy}&type=product`;

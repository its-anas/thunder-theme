...
document.querySelectorAll(".shopify-section:not(.main-header):not(.main-announcement-bar):not(.main-search-drawer):not(.main-cart-drawer):not(.main-popup):not(.main-quick-view)").forEach((section) => {
		...
		if (document.querySelector(".cart-drawer__products-list").childElementCount > 0) {
			setTimeout(() => {
				if (document.querySelector(".recommended-products__list-container").childElementCount > 0) {
					...
					document.querySelector(".recommended-products.mobile-only").style.display = "flex";
				}
			}, 1000);
		}
	}
...
function hideRecommendedDrawer() {
	if (document.querySelector(".recommended-products.desktop-only").classList.contains("active")) {
		...
		document.querySelector(".recommended-products.mobile-only").style.display = "none";
	}
}
	...
	await fetch("/cart.js")
		...
		.then((data) => {
			...
			data.items.forEach((item) => {
				...
				itemList.push(`
					<div class="cart-drawer__product reveal" data-variant="${item.variant_id}">
					...
					if (products.length > 0) {
						...
						recommendedProducts.forEach((product) => {
							...
							if (!cartProducts.some((cartProduct) => cartProduct.variant_id === variant_first_id)) {
								...
								document.querySelector(".recommended-products.desktop-only .recommended-products__list-container").innerHTML += `
									<div class="recommended-product reveal">
	...
	addProducts() {
		...
		if (products.length > 0) {
			...
			for (let product = 0; product < maxItems; product++) {
				...
				let quickViewButtonText = variants_size > 1 ? "Quick view +" : "Add to cart +";

			...
			for (const mutation of mutationList) {
				if (mutation.type === "childList" && mutation.target.className === "shopify-payment-button" && mutation.addedNodes.length > 0) {
					setTimeout(() => {
						...
						document.querySelector("product-page .shopify-payment-button__button").innerHTML += `<span></span> <span></span> <span></span> <span></span>`;
...
class ProductPageSlider extends HTMLElement {
	...
	connectedCallback() {
		...
		this.querySelector(".zoom-icon").addEventListener("click", () => {
			document.querySelector(".product-page__pictures").classList.toggle("zoom-in");
		});
